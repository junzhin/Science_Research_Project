...
class Solver(BaseSolver):
    ...
    def update_network(self, **kwargs):
        ...
        writer = SummaryWriter(log_dir = self.args.save_dir + '/logtrainresults/', comment = 'training')

        Total_losses_classification = 0.0
        Total_losses_SSL = 0.0
        Total_ssl_proportion_above_p_cut_off = 0.0
        Total_ssl_accs = 0.0
        ...
        for i in range(self.iters_per_epoch):
            ...
            Total_losses_classification += cls_loss.item()
            Total_losses_SSL += ssl_loss.item()
            
            Total_ssl_proportion_above_p_cut_off += mask
            Total_ssl_accs += acc_selected
